...
1bis. **Qualité corpus.** Lancer `npm run blog:quality -- --all` (standard `.claude/rules/article-parfait.md`) : erreurs bloquantes (H1 dans le corps, liens morts, silos cassés, descriptions hors gabarit…) et avertissements (maillage, métadonnées, GEO). C'est la source du backlog pour **/ameliorer-article**.
